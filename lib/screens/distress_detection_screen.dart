...
class _DistressDetectionScreenState extends State<DistressDetectionScreen>
  ...
  void _simulateSensorReadings() {
    Future.delayed(const Duration(seconds: 2), () {
      if (mounted && _isDetectionActive) {
        setState(() {
          // Randomly change sensor values
          _soundLevel = (0.3 + (0.7 * (DateTime.now().second % 10) / 10)).clamp(
            0.0,
            1.0,
          );
          _movementIntensity =
              (0.2 + (0.8 * (DateTime.now().millisecond % 100) / 100)).clamp(
                0.0,
                1.0,
              );

          if (_movementIntensity > 0.7) {
            _currentActivity = 'Running';
          } else if (_movementIntensity > 0.4) {
            _currentActivity = 'Walking';
          } else if (_movementIntensity > 0.1) {
            _currentActivity = 'Moving Slowly';
          } else {
            _currentActivity = 'Stationary';
          }

          // Check for distress patterns
          if (_soundLevel > 0.8 && _movementIntensity > 0.7) {
            _showDistressDialog();
          }
        });
